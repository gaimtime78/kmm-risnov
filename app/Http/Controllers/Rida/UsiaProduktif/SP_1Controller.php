...
use App\Exports\UsiaProduktif\SP_1\UsiaProduktifSP_1sExport;
use App\Imports\UsiaProduktif\SP_1\UsiaProduktifSP_1sImport;
use Maatwebsite\Excel\Facades\Excel;

class SP_1Controller extends Controller
{
    ...
    public function pilihperiode($fakultas)
    {
        ...
        $data = UsiaProduktifSP_1::select('periode', 'tahun_input', 'sumber_data')->distinct()->where('fakultas', $nama_fakultas)->get('periode', 'tahun_input', 'sumber_data');

        return view('admin.usiaproduktif.penelitipengabdisp_1.pilihperiode', ['data' => $data, 'nama_fakultas' => $nama_fakultas]);
    }
    ...
    public function details($nama_fakultas, $periode, $tahun_input)
    {
        ...
        $usiaproduktifsp_1 = UsiaProduktifSP_1::where([['fakultas', $fakultas], ['periode', $periode], ['tahun_input', $tahun_input]])->get();
        ...
        return view('admin.usiaproduktif.penelitipengabdisp_1.details', [
            'usiaproduktifsp_1' => $usiaproduktifsp_1, 'fakultas' => $fakultas,
    ...
    public function add()
    {
        return view('admin/usiaproduktif/usiaproduktifsp_1/add');
    }
    ...
    {
        $usiaproduktifsp_1 = UsiaProduktifSP_1::find($id);
        return view('admin/usiaproduktif/penelitipengabdisp_1/edit', compact('usiaproduktifsp_1'));
    }

    public function update(Request $request, $nama_fakultas, $periode, $tahun_input, $sumber_data)
    {
        $usiaproduktifsp_1 = UsiaProduktifSP_1::where([['fakultas', $nama_fakultas], ['periode', $periode], ['tahun_input', $tahun_input], ['sumber_data', $sumber_data]])->get();;
        foreach ($usiaproduktifsp_1 as $peneliti) {
        ...
        return redirect()->route('admin.usiaproduktifsp_1.pilihperiode', $nama_fakultas);
    }

    public function delete($nama_fakultas, $periode, $tahun_input)
    {
        $usiaproduktifsp_1 = UsiaProduktifSP_1::where([['fakultas', $nama_fakultas], ['periode', $periode], ['tahun_input', $tahun_input]])->get();
        foreach ($usiaproduktifsp_1 as $peneliti) {
            $peneliti->delete();
        }
    ...
    {
        return Excel::download(new UsiaProduktifSP_1sExport, 'penelitipengabdisp_1.xlsx');
    }

    public function import(Request $request)
    {
        $file = $request->file("usiaproduktifsp_1");
        // dd($file);
        if ($file !== null) {
            Excel::import(new UsiaProduktifSP_1sImport, $file);
        }

        UsiaProduktifSP_1::where('periode', 'kosong')
    ...
    {
        $peneliti = UsiaProduktifSP_1::find($id);
        // dd($peneliti);
        ...
        return redirect(route('admin.usiaproduktifsp_1.details', [$fakultas, $periode, $tahun]));
    }
}
