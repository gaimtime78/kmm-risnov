...
use App\Exports\UsiaProduktif\Magister\UsiaProduktifMagistersExport;
use App\Imports\UsiaProduktif\Magister\UsiaProduktifMagistersImport;
...
class MagisterController extends Controller
{
    ...
    public function pilihperiode($fakultas)
    {
        ...
        $data = UsiaProduktifMagister::select('periode', 'tahun_input', 'sumber_data')->distinct()->where('fakultas', $nama_fakultas)->get('periode', 'tahun_input', 'sumber_data');

        return view('admin.usiaproduktif.penelitipengabdimagister.pilihperiode', ['data' => $data, 'nama_fakultas' => $nama_fakultas]);
    }
    ...
    public function details($nama_fakultas, $periode, $tahun_input)
    {
        ...
        $usiaproduktifmagister = UsiaProduktifMagister::where([['fakultas', $fakultas], ['periode', $periode], ['tahun_input', $tahun_input]])->get();
        ...
        return view('admin.usiaproduktif.penelitipengabdimagister.details', [
            'usiaproduktifmagister' => $usiaproduktifmagister, 'fakultas' => $fakultas,
    ...
    public function add()
    {
        return view('admin/usiaproduktif/penelitipengabdimagister/add');
    }
    ...
    {
        $usiaproduktifmagister = UsiaProduktifMagister::where([['fakultas', $nama_fakultas], ['periode', $periode], ['tahun_input', $tahun_input], ['sumber_data', $sumber_data]])->get();;
        foreach ($usiaproduktifmagister as $peneliti) {
        ...
        return redirect()->route('admin.usiaproduktifmagister.pilihperiode', $nama_fakultas);
    }

    public function delete($nama_fakultas, $periode, $tahun_input)
    {
        $usiaproduktifmagister = UsiaProduktifMagister::where([['fakultas', $nama_fakultas], ['periode', $periode], ['tahun_input', $tahun_input]])->get();
        foreach ($usiaproduktifmagister as $peneliti) {
            $peneliti->delete();
        }
    ...
    {
        return Excel::download(new UsiaProduktifMagistersExport, 'penelitipengabdimagister.xlsx');
    }

    public function import(Request $request)
    {
        $file = $request->file("usiaproduktifmagister");
        // dd($file);
        if ($file !== null) {
            Excel::import(new UsiaProduktifMagistersImport, $file);
        }

        UsiaProduktifMagister::where('periode', 'kosong')
    ...
    {
        $peneliti = UsiaProduktifMagister::find($id);
        // dd($peneliti);
        ...
        return redirect(route('admin.usiaproduktifmagister.details', [$fakultas, $periode, $tahun]));
    }
}
