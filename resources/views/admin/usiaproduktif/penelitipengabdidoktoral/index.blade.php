      ...
      <div class="container">
        <div class="section">    
          ...
          <div id="table-datatables">
            <h4 class="header left">TABEL 1 @foreach ($nama_table as $name) {{ $name->nama_table }} @endforeach</h4>
            ...
            <div class="row">
              <div class="col s12 m12 l12">
                ...
                @foreach ($nama_table as $name) 
                  <form action="{{route('admin.usiaproduktifdoktoral.updateNamaTable', [$name->nama_table] )}}" method="post" enctype="multipart/form-data" >
                    @csrf
                ...
                <div id="import" class="modal">
                  <form action="{{route('admin.usiaproduktifdoktoral.import')}}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-content">
                      <h4>Import Data</h4>
                      <p>Untuk import data melalui excel, silahkan download template excel dan sesuaikan masukan</p>
                      <a href="{{asset('template\rida\table_1_usiaproduktifdoktoral.xlsx')}}">Download template</a>
                        <h5><label for="usiaproduktifdoktoral" class="form-label">Upload file excel</label></h5>
                        <input type="file" name="usiaproduktifdoktoral">
                ...
                <table id="data-menu" class="table display" cellspacing="0">
                  ...
                  <tbody>
                    @php
                      $i = 1;
                    @endphp
                      @foreach ($usiaproduktifdoktoral as $row)
                      <tr>
                          <td>{{$i}}</td>
                          <td>{{$row->fakultas}}</td>
                         
                          <td><a href="{{route('admin.usiaproduktifdoktoral.pilihperiode' , $row->fakultas )}}" class="btn" style="background-color: grey;">Detail</a></td>
                      </tr>
                      @php
                        $i++;
                      @endphp
