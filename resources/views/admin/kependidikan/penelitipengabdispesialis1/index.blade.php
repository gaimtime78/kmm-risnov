...
@section('css')
<link href="{{ asset('js/plugins/data-tables/css/jquery.dataTables.min.css') }}"
    type="text/css" rel="stylesheet" media="screen,projection">
@endsection

@section('js')
<script type="text/javascript"
    src="{{ asset('js/plugins/data-tables/js/jquery.dataTables.min.js') }}"></script>
<script type="text/javascript"
    src="{{ asset('js/plugins/data-tables/data-tables-script.js') }}"></script>
@endsection
            ...
            <div class="container">
                <div class="section">
                    ...
                    <div id="table-datatables">
                        <h4 class="header left">Tabel X @foreach ($nama_table as $name) {{ $name->nama_table }}
                            @endforeach</h4>
                        <div class="row">
                            <div class="col s12 m12 l12">
                                <a href="#import" class="waves-effect waves-light btn right modal-trigger"
                                    role="button">Upload Excel</a>
                                <a href="#update_nama_table" class="waves-effect waves-light btn right modal-trigger"
                                    role="button">Edit Nama Table</a>

                                {{-- Modal update nama table --}}
                                <div id="update_nama_table" class="modal">
                                    @foreach($nama_table as $name)
                                        <form
                                            action="{{ route('admin.penelitipengabdikependidikanspesialis1.updateNamaTable', [$name->nama_table] ) }}"
                                            ...
                                            <div class="modal-content">
                                                ...
                                                <div class="row">
                                                    <div class="input-field col s12">
                                                        <input value="{{ $name->nama_table }} " id="nama_table"
                                                            name="nama_table" type="text" class="validate" required>
                                ...
                                <div id="import" class="modal">
                                    <form
                                        action="{{ route('admin.penelitipengabdikependidikanspesialis1.import') }}"
                                        method="post" enctype="multipart/form-data">
                                        @csrf
                                        <div class="modal-content">
                                            <h4>Import Data</h4>
                                            <p>Untuk import data melalui excel, silahkan download template excel dan
                                                sesuaikan masukan</p>
                                            <a
                                                href="{{ asset('template\rida\table_7_peneliti_pengabdian_kependidikan_magister.xlsx') }}">Download
                                                template</a>
                                            <h5><label for="agendas" class="form-label">Upload file excel</label></h5>
                                            <input type="file" name="penelitipengabdispesialis1">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach($penelitipengabdispesialis1 as $row)
                                            <tr>
                                                <td>{{ $i }}</td>
                                                <td>{{ $row->fakultas }}</td>
                                                <td><a href="{{ route('admin.penelitipengabdikependidikanspesialis1.pilihperiode' , $row->fakultas ) }}"
                                                        class="btn" style="background-color: grey;">Detail</a></td>
                                            </tr>
                                            @php
                                                $i++;
                                            @endphp
