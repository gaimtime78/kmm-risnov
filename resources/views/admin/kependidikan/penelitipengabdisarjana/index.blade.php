      ...
      <div class="container">
        <div class="section">    
          ...
          <div id="table-datatables">
            <h4 class="header left">Tabel 9 RENTANG USIA PRODUKTIF PENELITI DAN PENGABDI TENAGA KEPENDIDIKAN  JENJANG SARJANA		
</h4>
            ...
            <div class="row">
              <div class="col s12 m12 l12">
                ...
                <!-- <a href="{{route('admin.penelitipengabdikependidikanprofesi.export')}}" class="waves-effect waves-light btn right" role="button">Export Excel</a> -->

                {{-- Modal import --}}
                <div id="import" class="modal">
                  <form action="{{route('admin.penelitipengabdikependidikansarjana.import')}}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-content">
                      <h4>Import Data</h4>
                      <p>Untuk import data melalui excel, silahkan download template excel dan sesuaikan masukan</p>
                      <a href="{{asset('template\rida\table_9_peneliti_pengabdian_kependidikan_sarjana.xlsx')}}">Download template</a>
                        <h5><label for="agendas" class="form-label">Upload file excel</label></h5>
                        <input type="file" name="penelitipengabdisarjana">
                ...
                <table id="data-menu" class="table display" cellspacing="0">
                  ...
                  <tbody>
                    @php
                      $i = 1;
                    @endphp
                      @foreach ($penelitipengabdisarjana as $row)
                      <tr>
                          <td>{{$i}}</td>
                          <td>{{$row->fakultas}}</td>
                          <td><a href="{{route('admin.penelitipengabdikependidikansarjana.pilihperiode' , $row->fakultas )}}" class="btn" style="background-color: grey;">Detail</a></td>
                      </tr>
                      @php
                        $i++;
                      @endphp
