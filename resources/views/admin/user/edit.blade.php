                            ...
                            <div style="padding:2em;" class="card-panel">
                                ...
                                <form action="{{ route('admin.user.update', [$data->id]) }}" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input value="{{$data->email}}" type="email" name="email" id="source" class="form-control"
                                            placeholder="Masukkan email user di sini">
                                    </div>
                                    <div class="mb-3">
                                        <h5><label for="source" class="form-label">New Password</label></h5>
                                        <input type="password" name="password" id="source" class="form-control"
                                            placeholder="Masukkan Password Baru user di sini">
                                    </div>
...
@section('js')
    ...
    <script>
        ...
        let defValRole = {!! json_encode($data->role)!!}
        let defValRoleList = defValRole.id
        initMulDrop({
            selector:'#roleDD',
            name:'role',
            label:'Select Role :',
            options:optList,
            initialValue:[defValRoleList],
            single:true
        })
    </script>
@endsection
